...
public class GateOpener extends AppCompatActivity
        implements NavigationView.OnNavigationItemSelectedListener {
    private TCPClient mTcpClient;
    private connectTask cTask;
    //WifiManager wifiManager = (WifiManager) getSystemService(Context.WIFI_SERVICE);
    //WifiInfo wifiInfo;
    Button One, Two, Three, Four, Five, Six, Seven, Eight, Nine, Zero, Clear, Connect, Enter, Mute;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.gate_opener);
            ...
            public void onClick(View v) {
                if(isCorrectSSID()) {
                    if (mTcpClient.isConnected())
                        Toast.makeText(getApplicationContext(), "You are already connected to the gate.", Toast.LENGTH_LONG).show();
                    else {
                        //cTask.cancel(true);
                        // if(cTask.isCancelled())
                        //cTask.execute();
                        mTcpClient.start("pi.jaredwines.com", 8000);
                    }
                }
                else
            ...
            public void onClick(View v) {
                if (passCode.length() == 4)
                    ...
                    if (mTcpClient.isConnected()) {
                        ...
                        if (mTcpClient != null) {
                            mTcpClient.sendMessage("gate " + passCode);
                        }
                        passCode = "";
                        PassCodeScreen.setText("****");
                    } else {
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_gate) {
            //Intent i = new Intent(getApplicationContext(),GateOpener.class);
            //startActivity(i);
        } else if (id == R.id.nav_tv) {

        } else if (id == R.id.nav_garage) {

        } else if (id == R.id.nav_lights_fan) {
    ...
    public class connectTask extends AsyncTask<String, String, TCPClient> {
        ...
        protected void onProgressUpdate(String... values) {
            ...
            2 = connecting to gate
            3 = connected to gate
            4 = disconnected from gate
             */
